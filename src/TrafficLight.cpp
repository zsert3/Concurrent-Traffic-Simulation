...
void TrafficLight::cycleThroughPhases()
{
    ...
    while (true)
    {
        ...
        // if this was not added the cycle duration would be between 4.001 and 6.001 seconds.
        std::this_thread::sleep_for(std::chrono::milliseconds(randomTime - 1));

        if(_currentPhase == TrafficLightPhase::green) _currentPhase = TrafficLightPhase::red;
        else _currentPhase = TrafficLightPhase::green;

        _msq.send(std::move(_currentPhase));
    }
